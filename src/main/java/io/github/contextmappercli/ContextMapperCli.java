...
public class ContextMapperCli {
    ...
    private static File createOutputFile() {
        File output = new File(generator.getFile());
        String separator = FileSystems.getDefault().getSeparator();
        String tmpdir = System.getProperty("java.io.tmpdir");
        output = new File(tmpdir + separator + output.getName() + "." + generator.getFormat().name().toLowerCase());
        return output;
    }

    private static void configureCommandLineParser() {
        options.addOption(Option.builder().longOpt(Args.FORMAT.name)
                .desc("Choose the output format. Default SVG. Valid formats are " + Arrays.stream(ContextMapFormat.values()).map(Enum::name).collect(Collectors.joining(",")))
                .hasArg()
                .build());

        options.addOption(Option.builder().longOpt(Args.LABELSPACINGFACTOR.name)
                .desc("Changes the spacing used to avoid label overlappings (factor between 1 and 20).")
                .hasArg()
                .build());

        options.addOption(Option.builder().longOpt(Args.WIDTH.name)
                .desc("Changes/fixes the width of the generated image.")
                .hasArg()
                .build());

        options.addOption(Option.builder().longOpt(Args.HEIGHT.name)
                .desc("Changes/fixes the height of the generated image.")
                .hasArg()
                .build());

        options.addOption(Option.builder().longOpt(Args.PRINTADDITIONALLABELS.name)
                .desc("Print additional labels for the relationship name and implementation technology.")
                .build());

        options.addOption(Option.builder().longOpt(Args.NOPRINTADDITIONALLABELS.name)
                .desc("Do not print additional labels for the relationship name and implementation technology.")
                .build());

        options.addOption(Option.builder().longOpt(Args.CLUSTERTEAMS.name)
                .desc("Cluster team BCs and generic BCs in the generated Context Map. Only relevant for team maps.")
                .build());

        options.addOption(Option.builder().longOpt(Args.NOCLUSTERTEAMS.name)
                .desc("Do not cluster team BCs and generic BCs in the generated Context Map. Only relevant for team maps.")
                .build());
    }
    ...
    private static void checkIncompatibleArgs(final CommandLine cmd) {
        if (cmd.hasOption(Args.HEIGHT.name) && cmd.hasOption(Args.WIDTH.name)) {
            ...
            helpFormatter.printHelp("java -jar context-mapper-cli.jar [OPTIONS] <FILE>", ContextMapperCli.options);
            System.exit(1);
        }
